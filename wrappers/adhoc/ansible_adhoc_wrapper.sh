#!/usr/bin/env bash
# -----------------------------------------------------------------------------------
#  ansible_adhoc_wrapper.sh
#  ==========================
# normal shell routine to trigger ansible execution
#
# Performs the following:
# 	1)  triggers ansible adhoc command executions
#
# Parameters :
#
...
usage() {
           ...
           echo "    --ansible_adhoc_cmd                                    - ansible adhoc command list . Default=${ansible_adhoc_cmd}"
...
get_params() {
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --ansible_adhoc_cmd) iansible_adhoc_cmd="$2"; shift 2;;
...
val_params()
{
        if [[ ! "${l_workspace}" || ! "${l_ansible_adhoc_cmd}"  || ! "${l_ansible_inventory}" || ! "${l_ansible_limit}"  || ! "${l_ansible_cfg}" || ! "${l_ansible_forks}" || ! "${l_ansible_user}" || ! "${l_extra_args}" || ! "${l_logs_dir}"  ]]; then
...
init_vars()
{
    ...
    l_ansible_adhoc_cmd=${iansible_adhoc_cmd-${ansible_adhoc_cmd}}
...
call_ansible()
{
    ...
    l_cmd_string="ansible all -m "${l_ansible_adhoc_cmd}" --limit "${l_ansible_limit}" -u ${l_ansible_user}  -f ${l_ansible_forks}"
